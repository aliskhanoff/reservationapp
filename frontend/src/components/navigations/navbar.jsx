...
import {TbLanguageKatakana, TbBrandBooking} from 'react-icons/tb'
...
export const Navbar = React.memo(({ isAuthenticated = false }) => {
    ...
    <nav className="bg-slate-100 py-4 border-gray-200 px-2 dark:bg-gray-800">
      <div className="container flex flex-wrap justify-between items-center mx-auto">
        <Link to="/" className="flex items-center">
            <i className='px-2 dark:text-white text-2xl'>
              <TbBrandBooking />
            </i>

            <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-slate-50 text-slate-800">{ applicationName }</span>
        </Link>
        ...
        <button onClick={ () => setIsOpen(prev => !prev) } data-collapse-toggle="navbar-default" type="button" className="inline-flex items-center p-2 ml-3 text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-default" aria-expanded="false">
          ...
          <svg className="w-6 h-6" aria-hidden={isOpen} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 15a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd"></path></svg>
        </button>
        ...
        <div className={ `w-full md:block md:w-auto ${ !isOpen && "hidden" }` } id="navbar-default">
          <ul className="flex flex-col p-4 mt-4 bg-slate-100 rounded-lg border border-slate-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-slate-100 dark:bg-gray-800 md:dark:bg-gray-800 dark:border-gray-700">
            ...
            <li className='flex text-2xl'>
              ...
              <i onClick={ () => setTheme(currentTheme === "light" ? "dark": "light") } className='hover:bg-blue-700 hover:text-white dark:text-white cursor-pointer rounded-full p-3 transition-colors duration-100'>
                    ...
                    t => (
                      <React.Fragment>
                        ...
                        <li className='flex items-center text-gray-700 hover:text-blue-500 mx-2 py-2 pr-4 pl-3 bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white'>
                          <a href='#' onClick={ logout } className="block mx-2 py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">{ t("LOGOUT_BUTTON") }</a>
                        </li>
